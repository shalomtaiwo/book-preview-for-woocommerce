...
function wbps_user_preview_simple()
{
    ...
    if (is_product()) {
        if ($empty_value_opt == 'wbps_txteditor') {
            front_wbps_button($product->get_id(), 'wbps_popup_btnsimple');
            include(plugin_dir_path(__FILE__) . '../template/simple/wbps-simple-editor-template.php');
        } elseif ($empty_value_opt == 'wbps_opt_pdf') {
            front_wbps_button($product->get_id(), 'wbps_popup_btnsimple_pdf');
            include(plugin_dir_path(__FILE__) . '../template/simple/wbps-simple-editor-template-pdf.php');
        } else {
            ?>
            <style>
...
function front_wbps_button($id, $class)
{
    ?><button style="background-color:<?php echo esc_attr(get_option("wbps_preview_front_settings_background_color")) ?>;
        ...
        class="wbps_popup_btn <?php echo $class ?> cursor-pointer mb-2 px-1 py-2 px-1 rounded border-0"
        data-bs-toggle="modal" data-bs-target="#wbps_<?php echo $id ?>">
        <span>
            <?php echo esc_html(get_option("wbps_preview_front_settings_title")) ?>
        </span><svg width="25px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
